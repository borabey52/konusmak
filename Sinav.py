...
try:
    if "GOOGLE_API_KEY" in st.secrets:
        ...
        st.error("Secrets ayarı bulunamadı. Lütfen Streamlit panelinden API Key ekleyin.")
...
def yapay_zeka_puanla_ve_duzelt(konu, detaylar, ham_metin):
    try:
        ...
        KONU: {konu}
        GİRİŞ: {detaylar['Giriş']}
        GELİŞME: {detaylar['Gelişme']}
        SONUÇ: {detaylar['Sonuç']}
        
        ÖĞRENCİ METNİ: {ham_metin}
        
        GÖREV:
        1. Metni noktalama ve imla kurallarına göre düzelt.
        2. Şu 4 kriteri 1-3 arası puanla:
           - Konu ve İçerik
           - Düzen
           - Dili Kullanma
           - Akıcılık
        ...
        return {"yuzluk_sistem_puani": 0, "duzeltilmis_metin": ham_metin, "ogretmen_yorumu": f"Model Hatası: {str(e)}"}
        ...
        if st.button("Bitir ve Puanla", type="primary", use_container_width=True):
            if not ad_soyad:
                ...
                with st.spinner("Puanlanıyor..."):
                    ham_metin = sesi_metne_cevir(ses_kaydi)
                    if ham_metin and "anlaşılamadı" not in ham_metin:
                        sonuc = yapay_zeka_puanla_ve_duzelt(secilen_konu, konular[secilen_konu], ham_metin)
                        
                        duzeltilmis = sonuc.get("duzeltilmis_metin", ham_metin)
                        puan = sonuc.get("yuzluk_sistem_puani", 0)
                        
                        sonuc_kaydet(ad_soyad, sinif_no, secilen_konu, duzeltilmis, puan, sonuc)
                        st.balloons()
                        ...
                        with st.expander("Sonuç Detayları", expanded=True):
                            ...
                            st.text_area("Düzenlenmiş Metin", duzeltilmis, height=150)
